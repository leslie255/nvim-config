...
function aleph.align_center(tbl, state)
...
function aleph.pad_margin(tbl, state, margin, shrink)
    local longest = longest_line(tbl)
    local left
    if shrink then
...
function aleph.highlight(state, end_ln, hl, left)
...
function aleph.resolve(to, el, opts, state)
...
function layout_element.text(el, conf, state)
    if type(el.val) == "table" then
        ...
        if conf.opts and conf.opts.margin and el.opts and (el.opts.position ~= "center") then
            local left
            val, left = aleph.pad_margin(val, state, conf.opts.margin, if_nil(el.opts.shrink_margin, true))
            padding.left = padding.left + left
        end
        if el.opts then
            if el.opts.position == "center" then
                val, _ = aleph.align_center(val, state)
            end
        ...
        if el.opts and el.opts.hl then
            hl = aleph.highlight(state, end_ln, el.opts.hl, padding.left)
        end
        state.line = end_ln
        return val, hl
    end
    ...
    if type(el.val) == "function" then
        return aleph.resolve(layout_element.text, el, conf, state)
    end
end
...
function layout_element.button(el, conf, state)
    ...
    if el.opts then
        if el.opts.position == "center" then
            local left
            val, left = aleph.align_center(val, state)
            if el.opts.align_shortcut == "right" then
                padding.center = padding.center + left
            end
            padding.left = padding.left + left
    ...
    if el.opts and el.opts.hl_shortcut then
        ...
        if el.opts.align_shortcut == "right" then
            hl = aleph.highlight(state, state.line, hl, #el.val + padding.center)
        else
            hl = aleph.highlight(state, state.line, hl, padding.left)
        end
    end
    ...
    if el.opts and el.opts.hl then
        ...
        list_extend(hl, aleph.highlight(state, state.line, el.opts.hl, left))
    end
...
function layout_element.group(el, conf, state)
    if type(el.val) == "function" then
        return aleph.resolve(layout_element.group, el, conf, state)
    end
...
function keymaps_element.group(el, conf, state)
    if type(el.val) == "function" then
        aleph.resolve(keymaps_element.group, el, conf, state)
    end
...
local function enable_alpha(conf)
    ...
    vim.cmd(noautocmd ..
    [[  silent! setlocal bufhidden=wipe nobuflisted colorcolumn= foldlevel=999 foldcolumn=0 matchpairs= nocursorcolumn nocursorline nolist nonumber norelativenumber nospell noswapfile signcolumn=no synmaxcol& buftype=nofile filetype=aleph nowrap

        augroup alpha_temp
        au!
        autocmd BufUnload <buffer> lua require('aleph').close()
        autocmd CursorMoved <buffer> lua require('aleph').move_cursor()
        augroup END
    ]])
    ...
    if conf.opts then
        if if_nil(conf.opts.redraw_on_resize, true) then
            vim.cmd([[
                autocmd alpha_temp VimResized * lua require('aleph').redraw()
                autocmd alpha_temp BufLeave,WinEnter,WinNew,WinClosed * lua require('aleph').redraw()
            ]])
        end
...
function aleph.start(on_vimenter, conf)
    local window = vim.api.nvim_get_current_win()

    aleph.move_cursor = function()
    ...
    local function draw()
        ...
        vim.api.nvim_buf_set_keymap(
            ...
            "n",
            "<CR>",
            "<cmd>lua require('aleph').press()<CR>",
        ...
        vim.api.nvim_buf_set_keymap(
            ...
            "n",
            "<M-CR>",
            "<cmd>lua require('aleph').queue_press()<CR>",
            { noremap = false, silent = true }
        )
        vim.api.nvim_win_set_cursor(state.window, cursor_jumps[ix])
    end
    aleph.redraw = draw
    aleph.close = function()
        cursor_ix = 1
        cursor_jumps = {}
        cursor_jumps_press = {}
        aleph.redraw = noop
...
function aleph.setup(config)
    ...
    vim.cmd([[
        command! Alpha lua require'aleph'.start(false)
        command! AlphaRedraw lua require('aleph').redraw()
        augroup alpha_start
        au!
        autocmd VimEnter * nested lua require'aleph'.start(true)
        augroup END
    ]])
end

aleph.layout_element = layout_element
aleph.keymaps_element = keymaps_element
